...
public class TC_Journey_02_TKTUpdateViaFULOnOSIReEntryDuringOverallCompletedJourney extends AppiumEngine{
	...
	public void verifyOysterTKTUpdateOnCompletedJourney(ITestContext context,Hashtable<String, String> data)
			throws Throwable {
		...
		try {
				...
				txn.copyFileToDGC(txnFilePath, AppConstants.DGC_PATH,action);
				txnFilePath = txn.generateTKTTransaction(context,data,TransactionFile2,TKTTransactionType,requestSequenceNumber,selectedDate,data.get("ZoneRanges"),action);
				txn.copyFileToDGC(txnFilePath, AppConstants.DGC_PATH,action);
