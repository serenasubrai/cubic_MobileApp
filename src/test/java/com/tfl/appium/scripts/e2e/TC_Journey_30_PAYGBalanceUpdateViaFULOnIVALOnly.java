...
public class TC_Journey_30_PAYGBalanceUpdateViaFULOnIVALOnly extends AppiumEngine{
	...
	public void verifyOysterPAYGBalanceUpdateOnIvalOnly(ITestContext context,Hashtable<String, String> data)
			throws Throwable {
		...
		Object EntryTransactionObject = JsonUtil.getJsonObject(TransactionData1, "$.IVAL");
		...
		try {
				...
				txnFilePath = txn.generateIvalTransaction(data,TransactionFile1,entryStationName1,IVALTransactionType,action);
				txn.copyFileToDGC(txnFilePath, AppConstants.DGC_PATH,action);		
				...
				balaneUpdate.verifyBalanceUpdatedtoMobile(context,data,action,txnDetailsMap.get("FinalBalance"));
				/*System.out.println("******* Waiting for IVAL transaction to upload to database ********");
				...
				journeyHistory.verifyIvalJourneyHistory(context,entryStationName1,exitStationName1,action);
				...
				if(curBal<=10) {
					...
					txn.copyFileToDGC(txnFilePath, AppConstants.DGC_PATH,action);
					txnFilePath = txn.generateIvalTransaction(data,TransactionFile1,entryStationName1,EntryTransactionType,action);
					txn.copyFileToDGC(txnFilePath, AppConstants.DGC_PATH,action);
					txn.verifyTrasactionExportedToCS(data,action);
					txnDetailsMap = txn.verifyTrasactionExportedToOYBO(data,action);
					txn.verifyFileSentToMobile(data, action,txnDetailsMap.get("FileReceivedId"));
